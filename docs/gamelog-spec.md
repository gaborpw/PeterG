...
**Status:** design spec, v0.1 — nothing built yet
...
### 3.6 Backlog — the killer utility
...
Surface on every game page:
- Friends who played it, with their ratings, above the global average.
- The completion/abandon funnel (*started → 5h → 20h → finished*), aggregated.
- Median hours to finish from real playthroughs — HowLongToBeat, but sourced from
  your own users, and correct because it's measured rather than self-reported.
...
- **Backend:** Go + Postgres. Single deployable binary, straightforward background
  workers for sync/import/aggregates, and it matches the stack already in use here.
- **API:** REST/JSON, cursor pagination, one public read API from day one (Letterboxd
  took a decade to ship an API and it cost them an ecosystem).
- **Web:** Next.js, server-rendered. Game and review pages must be indexable —
  organic search on *"[game name] review"* is the top acquisition channel and the
  reason Backloggd grew.
- **Mobile:** Backloggd's single biggest weakness is being web-only; GG's advantage
  is being mobile-first. Ship a good PWA at launch and native (React Native, shared
  API) by month 9. Logging happens on the couch, on a phone.
- **Search:** Postgres `pg_trgm` + `tsvector` to start; Typesense or Meilisearch
  when fuzzy title matching starts hurting (it will, around 100k titles).
- **Images:** cache and re-serve cover art through our own CDN with our own crops;
  never hotlink a third party's CDN in production.
- **Auth:** email/passkey, plus Steam OpenID (it doubles as an import).
- **Hosting:** Fly.io or Render + managed Postgres + S3-compatible storage. Boring
  on purpose.
...
Copy Letterboxd's structure, because it works and it keeps the product honest — the
free tier is fully usable and the paid tier sells *insight*, not access.
...
- **Pro (~$25/yr):** no ads, full stats, service filters (*"in my backlog and on
  Game Pass"*), leaving-soon and price-drop alerts, advanced filters, list cloning,
  data export.
- **Patron (~$60/yr):** custom profile/list art, extra stats, early features, a
  patrons directory.

No affiliate-link-driven storefront in v1. The moment the product is shaped by
commissions, the ratings stop being trustworthy, and trust is the only asset here.
...
|---|---|---|
| **M0 — Catalogue** | IGDB mirror, sync job, search, game pages | You can find any game and read its data |
| **M1 — Diary (single-player)** | Accounts, playthroughs, sessions, ratings, reviews, backlog, profile | The product is useful with zero other users |
| **M2 — Social** | Follow, feed, likes, comments, friend ratings on game pages, progress-gated spoilers | It's worth inviting someone |
| **M3 — Imports** | Steam first, then CSV migration, then PSN/Xbox | A new user sees their library in under 60s |
| **M4 — Discovery & retention** | Lists, stats, Year in Play, service/leaving-soon data | Organic growth loops exist |
| **M5 — Monetize & mobile** | Pro/Patron tiers, native apps, public API | Revenue and a real mobile experience |

M1 is the one that matters. If the private, single-player diary isn't good enough
to use alone, no amount of social will save it.

---

## 10. Risks and open questions

1. **Cold start.** A social network with no users is a spreadsheet. Mitigations, in
   order of value: Steam import (instant personal value), SEO-indexable game pages,
   an excellent Backloggd CSV migration, and Year in Play in December.
2. **Backloggd is entrenched** — ~650k users and the default answer to this exact
   question. We do not win on parity. We win on the playthrough model, honest
   abandonment data, progress-gated spoilers, and mobile.
3. **Metadata terms.** §5 — must be read directly before writing code against IGDB.
4. **Platform API fragility.** §6 — PSN/Xbox are unofficial; Nintendo is impossible.
5. **Moderation.** Gaming communities brigade. Budget for reporting, rate limits on
   new accounts, and a review-bomb damper (weight ratings by whether the rater
   actually logged play time — something we can do and Metacritic can't).
6. **Open design questions:**
   - Does a rating live on the playthrough or the game? *(Spec says playthrough —
     revisit once real replay volume exists.)*
   - Are DLCs separately ratable? *(Spec says yes. It may be clutter.)*
   - Does "ongoing" need a separate rating semantics — you can love a live-service
     game and still have no opinion about "finishing" it?
   - Do we surface hours-to-finish per user publicly, or only in aggregate?
...
through search-result summaries, not fetched in full. Anything below not covered by
these sources — the playthrough model, progress-gated spoilers, the schema, phasing
and the stack recommendation — is original design work or general knowledge, not
sourced from a retrieved page, and should be treated accordingly.
...
- Letterboxd — *Paid subscriptions*: https://letterboxd.com/about/pro/ (Pro/Patron feature split, streaming-service filters, custom art)
- Two Average Gamers — *Letterboxd for Games? We Tested 7*: https://www.twoaveragegamers.com/best-letterboxd-alternatives-for-gamers-who-track-everything-2026/ (Backloggd ~650k users and web-only; GG mobile-first and its console library sync)
...
- IGDB API docs — *Getting Started*: https://api-docs.igdb.com/ and APIs.io: https://apis.io/rate-limits/igdb/igdb-rate-limits/ (~4 req/s, 8 concurrent; Twitch client credentials)
